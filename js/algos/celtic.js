...
export default {
  ...
  name: 'Celtic Knot',
  category: 'Geometric',
  description: 'Interlaced Celtic plaitwork — woven cords passing over and under on a diagonal weave.',
  params: [
  ...
  create({ ctx, width, height, palette, params }) {
    ...
    function weave() {
      // oversize the field so a 45° rotation still covers the canvas
      ...
      for (let pass = 0; pass < 2; pass++) {
        for (let j = -1; j < rows; j++) {
          for (let i = -1; i < cols; i++) {
            ...
            const hOver = ((i + j) & 1) === 0; // horizontal over here?
